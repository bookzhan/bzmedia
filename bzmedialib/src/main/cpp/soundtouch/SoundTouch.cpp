...
void SoundTouch::setPitchOctaves(double newPitch) {
    ...
    BZLogUtil::logD("setPitchOctaves virtualPitch=%f", virtualPitch);
    calcEffectiveRateAndTempo();
}

void SoundTouch::setVirtualPitch(float pitch) {
    if (abs(virtualPitch - pitch) < 0.001f) {
        return;
    }
    virtualPitch = pitch;
    calcEffectiveRateAndTempo();
    BZLogUtil::logD("setVirtualPitch virtualPitch=%f", virtualPitch);
}
